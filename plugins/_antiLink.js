...
handler.before = async (m, { conn, isBotAdmin, isAdmin, isOwner,antiLink }) => {
  if (antiLink && m.isGroup && isBotAdmin && !isOwner) {
    ...
    var LTMPornhub = 'chat.whatsapp.com/' + (await conn.groupInviteCode('6282245496356-1602153905@g.us'))
    var LTMOfficial = 'chat.whatsapp.com/' + (await conn.groupInviteCode('6285892821182-1510584700@g.us'))
    if (!m.text.includes(linkGC) && !m.text.includes(LTMOfficial) && !m.text.includes(LTMPornhub) && m.text.match(/(chat.whatsapp.com)/gi)) {
