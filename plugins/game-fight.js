...
let handler = async (m, { conn, participants }) => {
  ...
  m.reply(`*Kamu* (level ${conn.level(global.DATABASE.data.users[m.sender].xp)[0]}) menantang *${conn.getName(lawan)}* (level ${conn.level(global.DATABASE.data.users[lawan].xp)[0]}) dan sedang dalam pertarungan sengit.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang.`)
