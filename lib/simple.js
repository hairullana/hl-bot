...
exports.WAConnection = (_WAConnection) => {
    ...
    status(jid, text, caption, thumb, tag) {
      return conn.sendMessage(jid, text, MessageType.text, {contextInfo: { mentionedJid: [tag] }, quoted: {key: {fromMe: false, participant: `0@s.whatsapp.net`, ...(jid ? { remoteJid: 'status@broadcast' } : {}) }, message: { "imageMessage": { "mimetype": "image/jpeg", "caption": caption, "jpegThumbnail": thumb} } }})
    }
...
`.trim()
    ...
    sendVcard(jid, name, number, quoted, options) {
    	let numb = number.replace(/[^0-9]/g, '')
		const detail = 'BEGIN:VCARD\n'
  + 'VERSION:3.0\n'
  + 'FN:' + name + '\n'
  + 'ORG:HL Bot;\n'
  + 'TEL;type=CELL;type=VOICE;waid=' + numb + ':' + PhoneNumber('+' + numb).getNumber('international') + '\n'
  + 'END:VCARD'
  		return this.sendMessage(jid, { displayname: name, vcard: detail }, MessageType.contact, { quoted, ...options })
	  }
